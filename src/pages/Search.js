...
class Search extends React.Component {
      ...
      this.setState(
        ...
        () => {
          ...
          if (this.state.params.artist_id) {
            axios
              .post(config.get("api_url") + "artists/artist-name-by-id", {
                id: this.state.params.artist_id,
              })
              .then((res) => {
                if (res.data.name) {
          ...
          if (this.state.params.track_gid) {
            axios
              .post(config.get("api_url") + "tracks/track-title-and-artist-by-gid", {
  ...
  render() {
    if (this.state.redirect_url) {
      return <Redirect to={encodeURI(this.state.redirect_url)} />;
    }
            ...
            <Form autoComplete='off' onSubmit={this.searchForSpins} style={{ marginBottom: 30 }}>
              ...
              <div className='cell'>
                <Button
                  ...
                  disabled={this.manageSubmitButton()}
                >
                  <Icon name='search' />
                  &nbsp;&nbsp;&nbsp;Search
                </Button>
