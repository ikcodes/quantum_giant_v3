  ...
  declareCommercials(trash_id, title, artist) {
    ...
    axios.post(config.get("api_url") + "commercials/add", postData).then((res) => {
      ...
      for (const elem of rowsToDisable) {
        ...
        i++;
        if (i === rowsToDisable.length) {
          // Once all elements have been removed...
          if (document.querySelectorAll(".spin-result-row").length === 0) {
            // If all the rows inside the table are gone...
              ...
              <Table.Body>
                ...
                {this.state.spins.map((spin) => {
                  ...
                  return (
                    <Table.Row
                      key={spin.prs_id}
                      className={"spin-result-row spin-result-" + trash_id}
                    >
                      ...
                      <Table.Cell collapsing>
                        <Button.Group basic size='small'>
                          <TableActionButton
                            buttonClass={"spin-result-button-" + trash_id}
                            link={false}
                            actionName={"This is a Commercial"}
                            actionIcon='ban'
                            onClick={() => {
                              this.declareCommercials(trash_id, spin.title, spin.artist);
                            }}
                          />
                        </Button.Group>
                      </Table.Cell>
