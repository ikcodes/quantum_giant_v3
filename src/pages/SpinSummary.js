	...
	componentDidMount(){
		...
		if(init_qv.weeks_back || init_qv.channel){
			...
			this.setState({	// Start out with this state, but also let it be updated later.
				...
				weeks_back: parseInt(init_qv.weeks_back) >= 0 ? parseInt(init_qv.weeks_back) : this.state.weeks_back,
			}, () => {
	...
	renderExportButton(){
		if(this.state.csvExport.length > 1){	// Must be a blank object to avoid crashing the plugin onload. Thus, length = 1. Re-render when this changes.
				var mkp =  <CSVLink 
					...
					>
					<Button style={{marginBottom: 30, width: '100%'}} primary>Export Spin Summary</Button>
				</CSVLink>;
				this.setState({
			...
			this.setState({
				exportButtonMarkup: <Button style={{marginBottom: 15, width: '100%'}} primary disabled>Loading Spin Summary...</Button>
			})
		}
	}
	...
	render(){
		...
		let spinCtMarkup = !this.state.loading ? 
				<Label size='large' style={spinCtStyle}>
					<Icon name='headphones'/>Total Spins: <strong>{ this.state.spinCount }</strong>
				</Label>
				:
				<Label size='large' style={spinCtStyle}>
					<Icon name='headphones' />Loading Spin Data...
				</Label>;
			...
			<Segment id="spinSummary" className="page">
				<div className="grid-x grid-margin-x">
					...
					<div id='queryCustomDatesContainer'>
						<Radio toggle checked={this.state.custom_range === true} onChange={this.state.custom_range === true ? this.deactivateCustomDates : this.activateCustomDates } label='Custom Dates' style={{marginBottom: 15}} />
					</div>
					...
					<div className="cell medium-5 large-4" style={displayWeeksShown}>
						<Input style={{marginBottom: 0}} label='Weeks Ago: ' fluid type='number' onChange={this.updateweeksBack} value={this.state.weeks_back} />
					</div>
					...
					<div className='cell medium-6' style={displayCustomDates}>
						...
						<Button style={{width: '100%'}} onClick={ this.updateCustom } primary>Query Custom Dates</Button>
					</div>
						...
						<Table celled compact striped unstackable>
							...
							<Table.Header>
								<Table.Row>
									...
									<Table.HeaderCell collapsing textAlign='center'>Spin Count</Table.HeaderCell>
									<Table.HeaderCell collapsing textAlign='center'>Spins</Table.HeaderCell>
									<Table.HeaderCell collapsing textAlign='center'>Rights</Table.HeaderCell>
								</Table.Row>
							</Table.Header>
							
							<Table.Body>
								<TableBodyLoader loading={ this.state.loading } cols={4} message="Loading Spin Summary..." slow={true} />
										...
										<Table.Row key={ index }>
											...
											<Table.Cell textAlign='center'>
												<Button.Group basic size='small'>
													<TableActionButton link={ '/summary/artist?artist_id='+spin_count['artist_id'] } actionName={ "Artist Summary for "+spin_count['artist_name'] } actionIcon='chart pie' />
													<TableActionButton link={ '/spins?artist_id=' + spin_count['artist_id'] } actionName={ "800PGR Spins by "+spin_count['artist_name'] } actionIcon='chart line' />
												</Button.Group>
											</Table.Cell>
											<Table.Cell textAlign='center'>
												<Button.Group basic size='small'>
													<TableActionButton link={ '/tracks/artist/'+spin_count['artist_id'] } actionName={ "Tracks by "+spin_count['artist_name'] } actionIcon='list' />
													<TableActionButton link={ '/albums/artist/'+spin_count['artist_id'] } actionName={ "Albums by "+spin_count['artist_name'] } actionIcon='clone' />
												</Button.Group>
											</Table.Cell>
										</Table.Row>
									);
