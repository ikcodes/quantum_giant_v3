...
class AlbumSummary extends React.Component{
	constructor(props){
		super(props);
		this.state = {
			...
			artist_id: 0,	// STAYS 0 FOR VARIOUS ARTISTS
			artist_name: '',
	...
	componentDidMount(){
		...
		if(init_qv.start_date && init_qv.end_date){
			...
			this.setState(updatedState, () => this.queryCustom())
		}
	...
	queryCustom(){
		...
		if(queryValues.album_id){
			this.setState({
				album_id: parseInt(queryValues.album_id),
				csvExport: [],
			}, () => {
				...
				axios.post(config.get('api_url') + 'albums/album-title-by-id', postData).then(res => {
					if(res.data.name){
						this.setState({
							...
							breadcrumbs_2_url: '/tracks/album/'+this.state.album_id
							...
							axios.post(config.get('api_url') + 'summary/album-custom', postData).then(res => {
	...
	queryForSummary(){
		...
		if(queryValues.album_id){
			this.setState({
				...
				axios.post(config.get('api_url') + 'albums/album-title-by-id', postData).then(res => {
					if(res.data.name){
						...
						var bc1_url = '/albums/artist/'+res.data.artist_id;
						var bc2_url = '/tracks/album/'+this.state.album_id;
						if(res.data.artist_id === 0){
							bc1_url = bc2_url
						}
						...
						this.setState({
							...
							artist_name: res.data.artist_name,
							breadcrumbs_1: res.data.artist_name,
							breadcrumbs_1_url: bc1_url,
							breadcrumbs_2: res.data.artist_name,
							breadcrumbs_2_url: bc2_url,
							...
							axios.post(config.get('api_url') + 'summary/album', summaryPost).then(res => {
								let newWeekText = this.convertWeeksToText(this.state.weeks_shown)
								if(res.data){
									this.setState({
										summary: res.data,
			...
			console.log("We need an album ID to not fuck this up!");
		}
	}
		...
		if(this.state.csvExport.length > 1){	// Must be a blank object to avoid crashing the plugin onload. Thus, length = 1. Re-render when this changes.
			return (
				<CSVLink data={this.state.csvExport}>
					<Button style={{marginBottom: 30, width: '100%'}} primary>Export Album Summary</Button>
				</CSVLink>
			);
		}else{
			return <Button style={{marginBottom: 15, width: '100%'}} primary disabled>Export Album Summary</Button>
		}
	}
		...
		if(this.state.summary.spins.length && !this.state.loading && this.state.custom_range !== true){
			...
			return(
				<div>
					...
					<p>Please adjust your criteria, or <a style={{textDecoration: 'underline'}}href={ '/search?album_title='+encodeURI(this.state.album_title) }>search for this album</a>.</p>
				</div>
			)
		}else{
			return (
		...
		return(
			<Segment id="spinSummary" className="page">
				<div className="grid-x grid-margin-x">
					<PageHeader
						h1='Album Summary'
						link={ '/summary/album?album_id='+this.state.album_id }
						iconName='headphones'
						instructions={ this.state.weeks_shown_text }
						breadcrumbs_1={ this.state.artist_name || 'Artist...' }
						breadcrumbs_1_url={ this.state.breadcrumbs_1_url || null }
						breadcrumbs_2={ this.state.album_title || '' }
					...
					<div className="cell small-12 table-scroll">
						<Table size='small' celled compact striped unstackable style={displayTableStyle}>
							...
							<Table.Body>
								<TableBodyLoader loading={ this.state.loading } cols={5} message="Loading Album Summary..." slow={true} />
