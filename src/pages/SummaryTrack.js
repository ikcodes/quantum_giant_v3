...
class TrackSummary extends React.Component{
	...
	queryForSummary(){
		...
		if(queryValues.track_gid){
					...
					if(res.data.title){
						this.setState({
							...
							axios.post(config.get('api_url') + 'summary/track', summaryPost).then(res => {
								let newWeekText = "Now viewing all spins in the " + this.convertWeeksToText(this.state.weeks_shown) + "."
								if(res.data){
									this.setState({
										summary: res.data,
										loading: false,
			...
			console.log("We need an GID to not fuck this up!");
		}
	}
		...
		if(this.state.csvExport.length > 1){	// Must be a blank object to avoid crashing the plugin onload. Thus, length = 1. Re-render when this changes.
			return (
				<CSVLink data={this.state.csvExport}>
					<Button style={{marginBottom: 30, width: '100%'}} primary>Export Track Summary</Button>
				</CSVLink>
			);
		}else{
			return <Button style={{marginBottom: 15, width: '100%'}} primary disabled>Export Track Summary</Button>
		}
	}
		...
		if(this.state.summary.spins.length && !this.state.loading){
			...
			return(
				<div>
					...
					<p>Please adjust your criteria, or <a style={{textDecoration: 'underline'}}href={ '/search?track_title='+encodeURI(this.state.track_title) }>search for this track</a>.</p>
				</div>
			)
		}else{
			return (
			...
			<Segment id="spinSummary" className="page">
				<div className="grid-x grid-margin-x">
					...
					<div className="cell medium-5 large-3">
						...
						{/* USER FILTERS */}
						<Input style={{marginBottom: 15}} label='Weeks:' fluid type='number' onChange={this.updateWeeksShown} value={this.state.weeks_shown} />
						<ChannelFiltering 
					...
					{/* SPIN TABLE */}
					<div className="cell medium-7 large-9">
					...
					<div className="cell small-12 table-scroll">
						<Table size='small' celled compact striped unstackable style={displayTableStyle}>
							...
							<Table.Body>
								<TableBodyLoader loading={ this.state.loading } cols={5} message="Loading Track Summary..." slow={true} />
