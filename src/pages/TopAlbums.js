...
class TopAlbums extends React.Component{
	constructor(props){
		...
		this.updateTopAlbums = this.updateTopAlbums.bind(this);
	...
	updateTopAlbums(){
		this.setState({
			...
			axios.post(config.get('api_url') + 'top-spinners/get-top-albums', postData).then(res => {
				if(res.data.summary){
					this.setState({
	...
	renderExportButton(){
		if(this.state.csvExport.length > 1){	// Must be a blank object to avoid crashing the plugin onload. Thus, length = 1. Re-render when this changes.
				var mkp =  <CSVLink 
					...
					>
					<Button style={{marginBottom: 15, marginTop: 15, width: '100%'}} primary>Export Top Albums</Button>
				</CSVLink>;
				this.setState({
			...
			this.setState({
				exportButtonMarkup: <Button style={{marginBottom: 15, marginTop: 15, width: '100%'}} primary disabled>Loading Top Albums...</Button>
			})
		}
	}
							...
							<div className="cell large-8">
								<div className="grid-x grid-margin-x" style={{position: "relative"}}>
									{/* <Segment style={{width: '100%'}}> */}
										...
										<div className='cell large-6' style={{marginTop: 15}}>
											{/* <Button id='queryCustomDates' onClick={ this.updateTopAlbums } primary>Query Custom Dates</Button> */}
											<Button style={{width: '100%'}} onClick={ this.updateTopAlbums } primary>Go!</Button>
										</div>
								...
								<Table celled compact striped unstackable style={tableStyle}>
									...
									<Table.Header>
										<Table.Row>
											...
											<Table.HeaderCell collapsing textAlign='center'>Summary</Table.HeaderCell>
										</Table.Row>
									</Table.Header>
									<Table.Body>
										<TableBodyLoader loading={ this.state.loading } cols={3} message="Loading Top Albums..." slow={true} />
										{ this.state.spinSummary.map( (spin_count, index) => {
											var summaryLink = '/summary/album?album_id='+spin_count['album_id'];
											return(
												<Table.Row key={ index }>
													<Table.Cell>{ spin_count['album_title'] }</Table.Cell>
													<Table.Cell textAlign='center'><strong>{ spin_count['spin_ct'] }</strong></Table.Cell>
													<Table.Cell textAlign='center'>
														<Button.Group basic size='small'>
															<TableActionButton link={ summaryLink } actionName={ "View Album Summary for "+spin_count['album_title'] } actionIcon='pie chart' />
														</Button.Group>
													</Table.Cell>
												</Table.Row>
											);
