...
class AlbumEdit extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      album: {
        // IF ADD, use this configuration (blank db record)
        album_title: "",
        album_is_single: false,
        album_is_compilation: false,
        album_release_date: "",
        album_upc: "", // Removed from FE 04.2023 - wait to remove from backend
        album_cat_num: "",
    ...
    if (this.state.album["album_title"] && debug === false) {
      axios.post(config.get("api_url") + "albums/update-album", postData).then((res) => {
        if (res.data.album) {
          ...
          const backTo = newAlbum["album_title"].length
            ? newAlbum["album_title"].substring(0, 1).toLowerCase()
            : "a";
          this.setState({
            ...
            instructions:
              "Successfully " +
              (this.state.add ? "add" : "updat") +
              "ed " +
              postData["album"]["album_title"] +
              "!",
            album: newAlbum,
  ...
  render() {
    ...
    let showReleaseDateClass = this.state.album["alt_release_dates"]
      ? "yes-release-date"
      : "no-release-date";
          ...
          <Form autoComplete='off'>
            ...
            <div className='field'>
              ...
              <Select
                ...
                value={
                  this.state.album["album_is_compilation"]
                    ? 0
                    : this.state.all_artists.filter(
                        (option) => option.value === this.state.album["artist_id"]
                      )
                }
            ...
            <div className='field'>
              ...
              <Select
                value={
                  this.state.album["label_id"]
                    ? this.state.all_labels.filter(
                        (option) => option.value === this.state.album["label_id"]
                      )
                    : 1
                }
            ...
            {/* ALT RELEASE DATES */}
            <div className={showReleaseDateClass}>{altReleaseDateMarkup}</div>
            <div>
              <Button
                onClick={this.addAlternateRelease}
            ...
            <Form.Group>
              <Form.Button
                onClick={(e) => {
                  this.saveAlbum(this.state.album["album_id"], e);
                }}
                primary
                size='small'
              >
              ...
              <Link to={"/tracks/album/" + this.state.album["album_id"]}>
                <Form.Button secondary size='small' style={{ marginRight: "12px" }}>
                  View Tracks on Album
                </Form.Button>
              </Link>
              <Link to={"/tracks/artist/" + this.state.album["artist_id"]}>
                <Form.Button secondary size='small'>
