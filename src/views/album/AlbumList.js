...
class AlbumList extends React.Component {
        ...
        axios.post(config.get("api_url") + "albums/load", postData).then((res) => {
          ...
          this.setState({
            ...
            breadcrumbs_1: res.data.breadcrumbs_1
              ? res.data.breadcrumbs_1
              : this.state.breadcrumbs_1,
            breadcrumbs_2: res.data.breadcrumbs_2
              ? res.data.breadcrumbs_2
              : this.state.breadcrumbs_2,
            breadcrumbs_1_url: res.data.breadcrumbs_1_url
              ? res.data.breadcrumbs_1_url
              : this.state.breadcrumbs_1_url,
            breadcrumbs_2_url: res.data.breadcrumbs_2_url
              ? res.data.breadcrumbs_2_url
              : this.state.breadcrumbs_2_url,
          });
        });
      }
    );
  ...
  deleteAlbum(album_id, album_title) {
    confirmAlert({
      ...
      message:
        "Really remove " + album_title + " from Quantum Giant, including all associated tracks?",
      buttons: [
        {
          label: "Yes",
          onClick: () => {
            ...
            axios.post(deleteUrl, postData).then((res) => {
              if (res.data.success >= 1) {
                ...
                alert(
                  "There was a problem deleting the album. Please refresh the page and try again."
                );
              }
            });
          },
  ...
  filterAlbums(letter, e) {
    axios
      .post(config.get("api_url") + "albums/albums-by-letter", { letter: letter })
      .then((res) => {
        this.setState({
          albums: res.data.albums,
        ...
        <div className='cell small-12 table-scroll'>
          <Table celled compact striped size='small' unstackable>
            ...
            <Table.Header>
              ...
              <Table.Row
                style={
                  this.state.current_artist || parseInt(this.state.current_label) >= 0
                    ? { display: "none" }
                    : {}
                }
              >
                  ...
                  <Table.Row key={album["album_id"]}>
                    ...
                    <Table.Cell collapsing>
                      <Button.Group basic size='small'>
                        ...
                        <TableActionButton
                          link={artistHref}
                          actionName={"View artist details for " + album["artist"]["artist_name"]}
                          actionIcon='user'
                        />
                      </Button.Group>
                    </Table.Cell>
