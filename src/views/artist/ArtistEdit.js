...
class ArtistEdit extends React.Component {
  ...
  componentDidMount() {
    // Get current artist (artist_id passed from ArtistPage)
    const postData = {
  ...
  editArtist(artist_id, e) {
    axios
      .post(config.get("api_url") + "artists/edit-artist", { artist_id: artist_id })
      .then((res) => {
        this.setState({
          action: "edit",
          artist: res.data.artist,
        ...
        <div className='cell large-7 medium-12'>
          <Form autoComplete='off'>
            ...
            <Form.Group>
              <Form.Button
                onClick={(e) => {
                  this.saveArtist(this.state.artist["artist_id"], e);
                }}
                primary
                size='small'
              >
