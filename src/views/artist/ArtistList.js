...
class ArtistList extends React.Component{
  ...
  componentDidMount(){
		axios.post(config.get('api_url')+'artists/load', {letter: this.state.current_letter}).then(res => {
	...
	deleteArtist(artist_id, artist_name){
		confirmAlert({
			...
			message: 'Really remove '+artist_name+' from Quantum Giant, including all associated albums and tracks?',
			buttons: [
				{
					label: 'Yes',
					onClick: () => {
						...
						axios.post(deleteUrl, postData).then(res => {
							if(res.data.success >= 1){
								...
								alert("There was a problem deleting the artist. Please refresh the page and try again.");
							}
						});
					}
				},
	...
	filterArtists(letter, e){
		axios.post(config.get('api_url')+'artists/artists-by-letter', {letter: letter}).then(res => {
			this.setState({
				artists: res.data.artists,
					...
					<Table celled striped compact size='small' unstackable>
						...
						<Table.Header>
							<Table.Row>
								...
								<Table.HeaderCell collapsing style={{textAlign:'left'}}>Spins</Table.HeaderCell>
								<Table.HeaderCell collapsing style={{textAlign:'left'}} colSpan={2}>Metadata</Table.HeaderCell>
							</Table.Row>
									...
									<Table.Row key={artistId}>
										...
										<Table.Cell collapsing>
											<Button.Group basic size='small'>
												<TableActionButton link={ '/summary/artist?artist_id='+artistId } actionName='View Artist Summary' actionIcon='chart pie' />
										...
										<Table.Cell collapsing>
											<Button.Group basic size='small'>
												<TableActionButton link={albumHref} actionName={ 'View Albums by ' + artist['artist_name'] } actionIcon='clone'/>
												<TableActionButton link={trackHref} actionName={ 'View All Tracks by ' + artist['artist_name'] } actionIcon='list'/>
											</Button.Group>
										</Table.Cell>
										<Table.Cell collapsing>
											<Button.Group basic size='small'>
												<TableActionButton link={artistHref} actionName='Edit Artist' actionIcon='edit'/>
												<TableActionButton link={false} actionName={ 'Delete Artist' } actionIcon='trash' onClick={ ()=> { this.deleteArtist(artist['artist_id'], artist['artist_name'])}}/>
											</Button.Group>
										</Table.Cell>
									</Table.Row>
								);
