...
class ChannelEdit extends React.Component{
	...
	componentDidMount(){	// Get current channel (channel_number passed from ChannelPage)
		const postData = {
			...
			axios.post(config.get('api_url')+'channels/edit-channel', postData).then(res => {
				if(res.data.channel){
					this.setState({
						...
						instructions: 'Now editing channel metadata for '+ res.data.channel['channel_name'] + '.',
					}, ()=> {
	...
	testChannel(){
		...
		axios.post(config.get('api_url')+'channels/test-channel', { channelKey: channelKey }).then((res) => {
			if(res.data.success){
				var channel_changed = parseInt(res.data.channelNumber) !== parseInt(this.state.channel.channel_number)
				var newChanData = {
				...
				this.setState({	// FAIL! Set state back to init
					success: false,
					channel: {
						channel_number: '',
						channel_name: '',
	...
	updateChannelNumber(e){
		...
		if(channelNumber >= 0){	// Fuck the cap
		...
		return(
			<div className="grid-x grid-margin-x">
				...
				<div className="cell small-12" style={{paddingBottom: 20}}>
					...
					<p className='smalltext'>Tracking a channel requires you to <strong>'guess' a Channel Key</strong>, using the Channel Name.<br className="show-for-medium"/>Remove spaces and test different capitalization patterns. Use the other channels as guides. <br className="show-for-medium"/>Click <strong>Test Channel</strong>. If you guess correctly, the 'Now Playing' table will fill in. Lastly, <strong>save the channel</strong>.</p>
				</div>
				...
				<div className="cell large-6 medium-12">
					<Form autoComplete='off'>
						...
						<div className='grid-x grid-margin-x'>
							<div className='cell'>
								<Input style={{marginBottom: 15}} label='Channel Number: ' fluid type='number' onChange={this.updateChannelNumber} value={this.state.channel.channel_number} />
							</div>
							<div className='cell'>
						...
						<Form.Group>
							<Form.Button onClick={(e) => this.testChannel() } secondary>Test Channel</Form.Button>
							<Form.Button onClick={(e) => this.saveChannel() } primary>Save Channel</Form.Button>
							<Link to={'/channels'}>
								<Form.Button primary style={{marginRight: '12px'}}>Back to Channels</Form.Button>
							</Link>
						</Form.Group>
					</Form>
				</div>
