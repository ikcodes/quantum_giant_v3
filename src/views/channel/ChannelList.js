...
class ChannelList extends React.Component{
	constructor(props){
		var today = new Date(),
    date = today.getFullYear() + '-' + ("0" + (today.getMonth() + 1)).slice(-2) + '-' + ("0" + (today.getDate())).slice(-2);	// FUCK. YOU. JAVASCRIPT.
		super(props);
		this.state = {
			channels: [],
			breadcrumbs_1: '',
					...
					onClick: () => {
						...
						axios.post(deleteUrl, postData).then(res => {
							if(res.data.success >= 1){
								...
								alert("There was a problem deleting the channel. Please refresh the page and try again.");
							}
						});
					}
				},
			...
			<div>
				<div className="grid-x grid-margin-x">
					...
					<div className='cell large-12' style={{marginBottom: 15}}>
						<p className='instructions'>Use this tool to <strong>modify which channels are tracked</strong> on SiriusXM.</p>
					</div>
				</div>
						...
						<Table celled striped unstackable>
							...
							<Table.Body>
								{ this.state.channels.map( channel => {
									...
									const currSpinsHref  = '/spins?channel='+channelNumber+'&start_date='+this.state.currentDate;
									...
									return (
										<Table.Row key={ channelNumber }>
											...
											<Table.Cell collapsing style={{textAlign: 'center'}}>{ channelTypeText }</Table.Cell>
											<Table.Cell collapsing style={{textAlign: 'center'}}>
												<Button.Group basic size='small'>
													<TableActionButton link={topSpinnersHref} actionName={ 'View Top Spinners on ' + channel['channel_name'] } actionIcon='sort amount down'/>
												</Button.Group>
											</Table.Cell>
											<Table.Cell collapsing style={{textAlign: 'center'}}>
												<Button.Group basic size='small'>
													<TableActionButton link={currSpinsHref} actionName={ 'View Recent Spins on ' + channel['channel_name'] } actionIcon='list'/>
												</Button.Group>
											</Table.Cell>
											<Table.Cell collapsing>
												<Button.Group basic size='small'>
													<TableActionButton link={channelHref} actionName='Edit Channel' actionIcon='edit'/>
													<TableActionButton link={false} actionName={ 'Delete Channel' } actionIcon='trash' onClick={ ()=> { this.deleteChannel(channel['channel_number'], channel['channel_name'])}}/>
												</Button.Group>
											</Table.Cell>
										</Table.Row>
									);
