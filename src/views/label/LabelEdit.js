...
class ChannelEdit extends React.Component{
	...
	componentDidMount(){	// Get current label (label_id passed from ChannelPage)
		const postData = {
    ...
    const currChannel = this.state.label;
		...
		// Do we have to make it this hard? There were a lot of condishes in there
	...
	// 	if(labelId >= 0){	// Fuck the cap
	// 		var tempChan = this.state.label;
	// 		tempChan['label_id'] = labelId;
	// 		this.setState({
	// 			label: tempChan,
			...
			<div className="grid-x grid-margin-x">
				...
				<div className="cell large-6 medium-12">
					<Form autoComplete='off'>
						...
						<Form.Group>
							<Form.Button onClick={(e) => this.saveLabel() } primary>Save Label</Form.Button>
							<Link to={'/label'}>
								<Form.Button primary style={{marginRight: '12px'}}>Back to Labels</Form.Button>
							</Link>
						</Form.Group>
					</Form>
				</div>
