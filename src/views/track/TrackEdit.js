...
class TrackEdit extends React.Component{
	...
	componentDidMount(){
		if(!this.state.add){
			if(this.state.track.track_gid){
				axios.post(config.get('api_url')+'tracks/load-track', { track_gid: this.state.track.track_gid }).then(res => {
					if(res.data.track){
						this.setState({
							track: res.data.track,
							breadcrumbs_1: res.data.track['track_title'],
							instructions: "Use the fields below to edit the metadata for "+res.data.track['track_title']+'.',
						}, ()=> {
							// console.log(this.state.track);
						});
					}else{ console.log("No track received here!"); }
				}).then( () => {	// Delay these to first load track data.
		...
		axios.get(config.get('api_url')+'albums/load-for-dropdown').then(res => {
			if(res.data.albums){
				...
				this.setState({
					...
					console.log('FUCK! Something is wrong here. Cant get albums.');
	...
	getDropdownArtists(){
		axios.get(config.get('api_url')+'artists/load-for-dropdown').then(res => {
			...
			}else{ console.log("NO RESPONSE FROM dropdownArtists"); }
		})
	}
		...
		axios.post(config.get('api_url')+'albums/edit-album', {album_id: album_id}).then(res => {
			if(res.data.album){
				...
				if(res.data.album_is_compilation){
					newTrack['artist_id'] = null;	// UI flag to fuck shit up here if you don't do it right
				}else{
					newTrack['artist_id'] = res.data.album.artist_id;
				}
				this.setState({ track: newTrack });
			}else{ console.log("Failed to get album with id: " + album_id); }
		});
	}
	
	updateBreadcrumbs(id, field){
		const endpoint = field === 'album_id' ? 'albums/album-title-by-id' : 'artists/artist-name-by-id'
	...
	updateAlbum = (event) => {
		...
		if(event){
			...
			console.log("FUCK! This is fucked!");
	...
	saveTrack(){
		...
		axios.post(config.get('api_url')+'tracks/save', postData).then(res => {
			if(res.data.track){
				...
				this.setState(prevState => {
					var newTrack = prevState.track;
					var instructions = "Successfully " + ( prevState.add ? 'add' : 'updat' ) + 'ed '+newTrack['track_title']+'!' ;
			...
			<div className="grid-x grid-margin-x">
				...
				<div className="cell large-7 medium-12">
					<Form autoComplete="off">
						...
						<div className="field">
							<div className={showSpellingClass}>
								{altSpellingMarkup}
							</div>
							
							<div id="altSpellingContainer">
								<Button onClick={ this.addAlternateSpelling } circular size='mini' icon='plus' style={{ marginTop: '7px' }}></Button>
						...
						<div className="field">
							<label>Album</label>
							<Select 
								value={ this.state.dropdownAlbums.filter(option => option.value === this.state.track['album_id'])}
						...
						<div className="field">
							...
							<Select 
								...
								value={ this.state.dropdownArtists.filter(option => option.value === this.state.track['artist_id'])}
						...
						*/}
						<Form.Group>
							<Form.Button onClick={(e) => { this.saveTrack()}} primary size='small'>Save Track</Form.Button>
							<Link to={'/tracks/album/'+this.state.track['album_id']}>
								<Form.Button secondary size='small' style={{marginRight: '12px'}}>Back to Album</Form.Button>
							</Link>
							<Link to={'/tracks/artist/'+this.state.track['artist_id']}>
								<Form.Button secondary size='small' style={{marginRight: '12px'}}>Back to Artist</Form.Button>
							</Link>
						</Form.Group>
					</Form>
				</div>
