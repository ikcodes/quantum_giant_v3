...
class TrackList extends React.Component{
  constructor(props){
	...
	// Dude... this is just bad lmao
	componentWillReceiveProps(nextProps) {
		window.location.reload();
	}
	
	componentDidMount(){	// What view to load? ( Album, Artist, or Letter )
		...
		axios.post(config.get('api_url')+'tracks/load', postData).then(res => {
			...
			this.setState({
				...
				breadcrumbs_2: res.data.breadcrumbs_2 ? res.data.breadcrumbs_2 : this.state.breadcrumbs_2,
					...
					// console.log("/tracks/load/ ---> COMPLETE");
					...
					onClick: () => {
						...
						axios.post(deleteUrl, postData).then(res => {
							if(res.data.success >= 1){
								...
								alert("There was a problem deleting the track. Please refresh the page and try again.");
							}
						});
					}
        },
  ...
  render(){
		...
		let hideNav = (
			this.state.current_album || this.state.current_artist 
		);
    ...
    return(
			<div className="grid-x grid-margin-x">
				...
				<ViewSummaryButton icon='edit' style={ editAlbumText && editAlbumHref ? {} : {display: 'none'}} link={ editAlbumHref } text={ editAlbumText }/>
				...
				<div className="cell small-12 table-scroll">
					<Table celled compact striped size='small' unstackable>
						...
						<Table.Header>
							<Table.Row>
								<Table.HeaderCell collapsing style={ hideNav ? {} : { display:'none'}}>#</Table.HeaderCell>
							...
							<Table.Row style={ hideNav ? { display:'none'} : {} }>
								<Table.HeaderCell textAlign='center' colSpan={ hideNav ? '6' : '5' } id="artistsLetterMenu">
						...
						<Table.Body>
							...
							<TableBodyLoader loading={ this.state.loading } slow={false} cols={6} message="Loading Tracks..."/>
							...
							{this.state.tracks.map( track => {
								...
								if(this.props.activeAlbum){
									rightsMarkup= <Button.Group basic size='small'>
																	<TableActionButton link={ artistHref } actionName={ artistAction } actionIcon='user' />
																</Button.Group>
								}else if(this.props.activeArtist){
									rightsMarkup= <Button.Group basic size='small'>
																	<TableActionButton link={ albumHref } actionName={ albumAction } actionIcon='clone' />
																</Button.Group>
								}else{
									rightsMarkup= <Button.Group basic size='small'>
																	<TableActionButton link={ albumHref } actionName={ albumAction } actionIcon='clone' />
								...
								return (
									<Table.Row key={ track['track_gid'] }>
										<Table.Cell style={ hideNav ? {} : { display:'none'} } collapsing>{track['track_num'] || ''}</Table.Cell>
										<Table.Cell>{ track['track_title'] || ''}</Table.Cell>
										
										{/* Link style is out */}
										{/* <Table.Cell><a href={artistHref}>{ track['artist_name'] || ''}</a></Table.Cell> */}
										{/* <Table.Cell><a href={albumHref}>{ track['album_title'] || ''}</a></Table.Cell> */}
										
										{/* Just list, advanced actions. */}
										...
										{/* <Table.Cell> */}
											{/* <Button.Group basic size='small'> */}
												{/* <TableActionButton link={ spinHref } actionName='View Spins' actionIcon='headphones' /> */}
												{/* <TableActionButton link={ searchHref } actionName='Search For This Track' actionIcon='search' /> */}
											{/* </Button.Group> */}
										{/* </Table.Cell> */}
										<Table.Cell collapsing>
											<Button.Group basic size='small'>
												<TableActionButton link={ '/summary/track?track_gid='+track['track_gid'] } actionName='View Track Summary' actionIcon='chart pie' />
											</Button.Group>
										</Table.Cell>
										<Table.Cell>
											{ rightsMarkup }
										...
										<Table.Cell>
											<Button.Group basic size='small'>
												<TableActionButton link={ editHref } actionName='Edit Track' actionIcon='edit' />
												<TableActionButton link={false} actionName={'Delete Track'} actionIcon='trash' onClick={ () => { this.deleteTrack(track['track_gid'], track['track_title'])}}/>
											</Button.Group>
										</Table.Cell>
									</Table.Row>
								);
